...
def get_cfss(
    ...
    stock_ids: array?
    '''
    list_of_dicts = rawCrud.get_cfss_with_filter(
...
@router.post(
    "/cfs/",
    response_model=schemas.CFS
)
def create_cfs(
        obj_create: schemas.CFSCreate,
        db: Session = Depends(get_db)
):
    ...
    db_kbar = crud.get_cfs_by_stock_and_date(
    ...
    return crud.create_cfs(db=db, obj=obj_create)


@router.post(
    "/composite/cfs/",
    response_model=List[schemas.CFS]
)
def create_cfss(obj_creates: List[schemas.CFSCreate], db: Session = Depends(get_db)):
    '''
    * 只要有一個record duplicate，全部都取消
    ...
    dupli_kdays = crud.list_cfs_dupli(db=db, objs=obj_creates)
    if dupli_kdays:
        # 要不要直接create沒有重複的？
        raise HTTPException(
    ...
    # print(obj_create.high, obj_create.low, '\n\n\n')
    print('creating')
    return crud.create_cfss(db=db, objs=obj_creates)
    # for i in obj_creates:
